...
try:
    from prometheus_client import (  # type: ignore
        CollectorRegistry, Gauge, start_http_server,
    )
    _HAS_PROM = True
except Exception:
    _HAS_PROM = False
...
class StepRecord:
    ...
    # v0.3 collective sub-fields (also live inside ``collective`` dict for
    # backward-compat; surfaced here for type-checked access)
    # Note: these are NOT dataclass fields — they are read/written through
    # the ``collective`` dict to keep the on-wire JSON flat and compatible
...
class WandBSink:
    ...
    def __init__(
        ...
        try:
            ...
            self._active = True
            log.info("WandB run initialised: %s", _wandb_lib.run.url if _wandb_lib.run else "unknown")
...
class PrometheusExporter:
    ...
    def update(self, rec: "StepRecord") -> None:
        ...
        self._g["moe_expert_compute_ms"].set(
            rec.collective.get("expert_compute_ms", 0.0))
