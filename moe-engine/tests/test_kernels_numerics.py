...
from pkg.kernels.moe_router import TRITON_AVAILABLE, moe_topk_route
...
class TestMoERouterNumerics:
    ...
    def test_forward_indices_match_reference(self, H, E, K):
        ...
        idx_triton, w_triton = moe_topk_route(tokens, gate_w, K, force_reference=False)
        ...
        assert (idx_triton == idx_ref).all(), (
            f"Expert indices differ; Triton: {idx_triton[0]}, Ref: {idx_ref[0]}"
        )
    ...
    def test_backward_grad_tokens_match_reference(self, H, E, K):
        ...
        grad_gate_w_tri = gate_w.grad.clone().detach()  # noqa: F841
        ...
        grad_gate_w_ref = gate_w_ref.grad.clone().detach()  # noqa: F841

        # Compare
        assert_close(grad_tokens_tri, grad_tokens_ref, name="grad_tokens", atol=1e-5, rtol=1e-5)
    ...
    def test_backward_grad_gate_w_match_reference(self, H, E, K):
        ...
        # Compare
        assert_close(grad_gate_w_tri, grad_gate_w_ref, name="grad_gate_w", atol=1e-5, rtol=1e-5)
    ...
    def test_backward_gradients_randomized_triton(self):
        ...
        for seed in range(50):
            ...
            assert_close(
                grad_tokens_tri,
                tokens_ref.grad.clone().detach().double(),
                name=f"grad_tokens_randomized_seed_{seed}",
                atol=1e-5,
                rtol=1e-5,
            )
            assert_close(
                grad_gate_w_tri,
                gate_w_ref.grad.clone().detach().double(),
                name=f"grad_gate_w_randomized_seed_{seed}",
                atol=1e-5,
                rtol=1e-5,
            )
