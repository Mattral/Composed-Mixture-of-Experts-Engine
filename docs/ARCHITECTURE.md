...
The following diagrams use [Mermaid](https://mermaid.js.org/) syntax
(rendered automatically on GitHub and most doc platforms).
...
sequenceDiagram
    ...
    participant SM as ClusterStateMachine
    participant AC as AsyncCheckpointer
    participant TE as TorchElastic

    TL->>HM: health_check() every 50 steps
    HM-->>TL: dead_ranks = [rank_3]

    TL->>SM: transition(RUNNING → DRAINING)
    SM-->>TL: drain in-flight micro-batches

    TL->>AC: flush() — wait for NVMe write
    AC-->>TL: NVMe checkpoint committed (atomic rename)

    TL->>TE: signal rendezvous reset
    TE->>TE: restart rank_3, re-init process group

    TL->>SM: transition(DRAINING → RECOVERING)
    SM->>SM: new_ep_size = _largest_divisor_le(E, new_world_size)
    SM->>SM: reshard experts round-robin to surviving ranks

    TL->>AC: load(model, optimizer, latest_step)
    AC-->>TL: model loaded from NVMe tier (~seconds)

    TL->>SM: transition(RECOVERING → RESUMED)
    TL->>TL: training continues (reduced world_size)

    Note over AC: Background: S3 upload of post-recovery checkpoint
```

### Async Two-Tier Checkpoint Sequence

```mermaid
sequenceDiagram
    participant TL as Training Loop
    participant AC as AsyncCheckpointer
    participant NVMe as LocalNVMeAdapter
    participant S3 as S3Adapter
    participant BG as Background Thread

    TL->>AC: save(model, optimizer, step=500)
    AC-->>TL: returns immediately (non-blocking)

    TL->>TL: step 501, 502, ... (training continues)

    AC->>BG: enqueue I/O task
    BG->>NVMe: D2H copy → serialize → write .tmp
    NVMe->>NVMe: fsync (O_DIRECT)
    NVMe->>NVMe: atomic rename .tmp → ckpt_500.pt
    NVMe->>NVMe: prune oldest (retain last N)
    BG-->>AC: Tier-1 commit complete

    BG->>S3: multipart upload ckpt_500.pt
    S3->>S3: write .meta.json (step, rank, ts, hash)
    S3->>S3: prune remote (retain last N)
    BG-->>AC: Tier-2 commit complete (durable)
```

### MoE Layer Forward Sequence (single rank, ep_size=1)

```mermaid
sequenceDiagram
    participant TL as Training Loop
    participant ML as DistributedMoELayer
    participant RI as MoERouterInterface
    participant RK as Triton Kernel
    participant EP as ExpertParallel (A2A)
    participant EX as _SwiGLUExpert (FFN)

    TL->>ML: forward(tokens [B,S,H])
    ML->>ML: reshape → flat [N, H]
    ML->>RI: forward(flat)
    RI->>RK: _router_fwd_kernel(tokens, gate_w)
    RK-->>RI: idx [N,K], weights [N,K], dispatch_cnt [E]
    RI->>RI: assert conservation: sum(dispatch_cnt) == N*K
    RI-->>ML: RouterStats(idx, weights, dispatch_cnt, ...)

    ML->>ML: sort tokens by expert → tokens_sorted [N*K, H]
    ML->>EP: all_to_all_dispatch(tokens_sorted, send_counts)
    Note over EP,EX: CUDA streams overlap: dispatch ∥ expert FFN
    EP-->>ML: received [total_recv, H]  (dedicated stream)

    ML->>EX: expert_ffn(received)       (default stream)
    EX->>EX: silu(gate(x)) * up(x) → down(·)
    EX-->>ML: expert_out [total_recv, H]

    ML->>EP: all_to_all_combine(expert_out, ...)
    EP-->>ML: combined_sorted [total_send, H]

    ML->>ML: weighted scatter → output [N, H]
    ML->>ML: assert not NaN
    ML->>ML: reshape → [B, S, H]
    ML-->>TL: output [B, S, H]
```
...
```mermaid
sequenceDiagram
    participant User as User / train.py
    participant LC as load_config() / from_yaml()
    participant PY as Pydantic v2
    participant EC as EnvOverrides

    User->>LC: MoEConfig.from_yaml("configs/default.yaml")
    LC->>LC: Path.exists() check → FileNotFoundError if missing
    LC->>LC: yaml.safe_load() → dict
    LC->>EC: _apply_env_overrides(d)
    EC->>EC: scan os.environ for MOE_<SECTION>__<KEY>
    EC-->>LC: dict with overrides applied

    LC->>PY: MoEConfig(**d)
    PY->>PY: ModelConfig validation (top_k ≤ E, hidden%8==0, dtype valid)
    PY->>PY: TrainingConfig validation (warmup < max_steps)
    PY->>PY: CheckpointConfig validation (remote_uri prefix)
    PY->>PY: ElasticConfig validation (min_nodes ≤ max_nodes)

    alt Validation passes
        PY-->>LC: MoEConfig instance
        LC-->>User: cfg (fully validated, typed)
    else Validation fails
        PY-->>LC: ValidationError (field path + message)
        LC-->>User: ConfigValidationError (clear, actionable message)
    end
```
